...
def run_analysis(league_id):
    ...
    return {
    "df_footballers": df_footballers,
    "df_teams": df_teams,
    "all_playing_XI_players_df": all_playing_XI_players_df,
    "rule_based_metrics": rule_based_metrics,
    "best_transfer_str": best_transfer_str,
    "worst_transfer_str": worst_transfer_str,
    "rise_strs": rise_strs,
    "fall_strs": fall_strs,
    "top_3_highest_scoring_captains_df": top_3_highest_scoring_captains_df,
    "bottom_3_lowest_scoring_captains_df": bottom_3_lowest_scoring_captains_df,
    "league_name_text": league_name_text,
    "gw_text": gw_text,
    "current_gw": current_gw,
    "df_teams_times_captained_strip_str": df_teams_times_captained_strip_str,
    "df_teams_times_chosen_strip_str": df_teams_times_chosen_strip_str,
    "chip_usage_stats_str": chip_usage_stats_str,
    "top_unique_picks": top_unique_picks,
    "top_scoring_rare_captains_str": top_scoring_rare_captains_str,
    "chip_usage_stats": chip_usage_stats,
    "real_gameweek_results_str": real_gameweek_results_str,
    "favourite_teams_str": favourite_teams_str,
    "rival_1": rival_1,
    "rival_2": rival_2,
    "top_3_reliance_str": top_3_reliance_str,
    "bottom_3_reliance_str": bottom_3_reliance_str,
    "top_3_chips_str": top_3_chips_str,
    "bottom_3_chips_str": bottom_3_chips_str,
    "top_3_chips_df": top_3_chips_df,
    "bottom_3_chips_df": bottom_3_chips_df,
    "top_captaincy_str": top_captaincy_str,
    "worst_captaincy_str": worst_captaincy_str,
    "top_3_highest_scoring_autosubs_str": top_3_highest_scoring_autosubs_str,
    "bottom_3_lowest_scoring_autosubs_str": bottom_3_lowest_scoring_autosubs_str,
    "top_3_highest_scoring_vc2c_str": top_3_highest_scoring_vc2c_str,
    "bottom_3_lowest_scoring_vc2c_str": bottom_3_lowest_scoring_vc2c_str,
    "rule_based_metrics_text": rule_based_metrics_text,
    "df_rare_players_shortlisted": df_rare_players_shortlisted,
    "top_scoring_rare_captains_df": top_scoring_rare_captains_df,
    "all_teams_display_ranking": all_teams_display_ranking
    }
...
def run_ai_calls():
    ...
    scout_predictions = run_agent(
        fpl_agent_links,
        f"""Extract only tips/advice/suggestions from official FPL Scout articles for Gameweek {results["current_gw"]} of the 2025/26 season. 
    Rules you must follow strictly:
    - Only include articles published August 2025 or later.
    - Only include articles that explicitly say they are for Gameweek {results["current_gw"]}.
    ...
    Here is the data for the captains of our league's teams: {results["df_teams_times_captained_strip_str"]}. 
    ...
    Similarly, here's the data for commonly chosen footballers by our managers: {results["df_teams_times_chosen_strip_str"]}. In the last column,
    ...
    Also here are the chip usage statistics by each team for this week: {results["chip_usage_stats_str"]}. Look at them, and comment whether there was any chip
    ...
    prompt_rare_choices = f"""Give shoutout to managers who played rarely chosen players and scored well. Use this data: {results["top_unique_picks"]}. 
    Also, here are rarely chosen captains and their scores: {results["top_scoring_rare_captains_str"]}. 
    ...
    Try to comment about the opponent difficulty.
    Check {results["chip_usage_stats"]} to see if anyone stood out with chip usage this week. Limit your response within 50-100 words.
    When discussing managers, don't take their full names, just the first name is enough.
    Important FPL rule:FPL rewards double points for the captain by default, so if the data says a captain scored 5 points, those are 5 raw points before doubling, i.e., the team actually got 10 points from him.
    """
    ...
    prompt_fixture_results = f"""Use the real gameweek results to trash talk about managers' favourite teams. Here are the results: {results["real_gameweek_results_str"]}. 
    Here are managers and their favourite teams: {results["favourite_teams_str"]}. Only comment if a result is surprising/unexpected, or if a team won by a big margin, or if it's usual to make fun of a team everytime they play bad.
    ...
    prompt_reliance = f"""Use this data to comment about players who relied the most on a single player for their scores: {results["top_3_reliance_str"]}
    Similarly, use this for players who relied the least on a single player for their scores: {results["bottom_3_reliance_str"]}. 
    ...
    prompt_chips = f"""Here are some statistics about the points earned by managers using chips. Top chip users throughout the season: {results["top_3_chips_str"]}.
    Worst chip users throughout the season: {results["bottom_3_chips_str"]}.
    ...
    results["top_3_chips_df"]["Chips used with gameweek"] = results["top_3_chips_df"]["Overall chips used"].apply(clean_chips)
    results["bottom_3_chips_df"]["Chips used with gameweek"] = results["bottom_3_chips_df"]["Overall chips used"].apply(clean_chips)
    top_3_chips_df = results["top_3_chips_df"].drop(columns=["Overall chips used"])
    bottom_3_chips_df = results["bottom_3_chips_df"].drop(columns=["Overall chips used"])
    top_3_chips_str = top_3_chips_df.to_string(index=False)
    bottom_3_chips_str = bottom_3_chips_df.to_string(index=False)

    prompt_captaincy = f"""Comment on how well someone chose their captain (one should choose their highest scoring player as captain always). A ratio of 1 means the best player chosen as captain. 0 means worst. Check these tables: {results["top_captaincy_str"]}, {results["worst_captaincy_str"]}.
    ...
    prompt_rise_fall = f"""These are the teams with the highest increase in rank to reach to their current rank: {results["rise_strs"]}. 
    These are the teams with the highest fall in rank to reach their current rank: {results["fall_strs"]}.
    ...
    prompt_luck = f"""Luck factor: Here are the teams with most points from automatic-substitutions: {results["top_3_highest_scoring_autosubs_str"]}. Here are the teams with least points from automatic-substitutions: {results["bottom_3_lowest_scoring_autosubs_str"]}.
    Here are the teams with most points from Vice Captain acting as Captain: {results["top_3_highest_scoring_vc2c_str"]}. Here are the teams with least points from Vice Captain acting as Captain: {results["bottom_3_lowest_scoring_vc2c_str"]}. 
    ...
    top_team_season_ranking_history = results["df_teams"][["Rankings history", "Team name", "Player name"]].head(1)
    bottom_team_season_ranking_history = results["df_teams"][["Rankings history", "Team name", "Player name"]].tail(1)
    ...
    {results["rule_based_metrics_text"]}
    ...
    footballers_scores = results["df_footballers"][["Footballer name", "GW points"]]
    footballers_scores_str = footballers_scores.to_string(index=False)

    df_rare_players_shortlisted_stripped = results["df_rare_players_shortlisted"].drop(columns=["Last opponent name", "Last opponent strength (1-10)"])
    top_unique_picks_stripped = df_rare_players_shortlisted_stripped.to_string(index=False)

    top_scoring_rare_captains_df_stripped = results["top_scoring_rare_captains_df"].drop(columns=["Last opponent name", "Last opponent strength (1-10)"])
    ...
    MUST print these two tables in your output immediately with the above content. 1) Title: Most single-player reliant teams. Data: {results["top_3_reliance_str"]}. 2) Title: Least single-player reliant teams. Data: {results["bottom_3_reliance_str"]}.
    ...
    MUST print these two tables in your output immediately with the above content. 1) Title: Top captaincy effectiveness teams. Data: {results["top_captaincy_str"]}. 2) Title: Bottom captaincy effectiveness teams. Data: {results["worst_captaincy_str"]}.
    ...
    MUST print these FOUR tables in your output immediately with the above content. 1) Title: Most points earned by auto-subs, Data: {results["top_3_highest_scoring_autosubs_str"]}. 2)Title: Least points earned by auto-subs. Data: {results["bottom_3_lowest_scoring_autosubs_str"]}. 3) Title: Most points earned by Vice Captain acting as Captain. Data: {results["top_3_highest_scoring_vc2c_str"]}. 4) Title: Least points earned by Vice Captain acting as Captain. Data: {results["bottom_3_lowest_scoring_vc2c_str"]}.
    ...
    Read through these tips: {scout_predictions.final_output}
...
if run and league_id:
    with st.spinner("Running heavy analysis..."):
        st.session_state["results"] = run_analysis(league_id)

if run and league_id:
    with st.spinner("Running AI Calls..."):
        st.session_state["ai_calls"] = run_ai_calls()
...
if "results" in st.session_state:
    results = st.session_state["results"]
    ai_calls = st.session_state["ai_calls"]
    report_lines = []
    ...
    if show_internal:
        ...
        st.dataframe(results["df_footballers"].iloc[400:450])
        st.write("Teams Dataframe")
        st.dataframe(results["df_teams"].astype(str))
        st.write("All footballers playing for our managers")
        st.dataframe(results["all_playing_XI_players_df"])
        st.write("Rule based metrics list")
        st.write(results["rule_based_metrics"])
    ...
    st.markdown(f"\n #### {results["league_name_text"]}")
    st.markdown(f"\n #### {results["gw_text"]}")
    st.markdown(f"### Summary: \n\n""")
    st.markdown(f"{ai_calls["response_final"].choices[0].message.content} \n\n")

    report_lines.append(f"### {results["league_name_text"]}")
    report_lines.append(f"### {results["gw_text"]}")
    report_lines.append(f"Summary:\n\n{ai_calls["response_final"].choices[0].message.content}\n\n")

    st.text(f"\nLeague Table Standings\n")
    report_lines.append("\nLeague Table Standings\n")

    # Safe handling for all_teams_display_ranking (paste this block where you used the old code)
    df_ranking = results.get("all_teams_display_ranking")

    if df_ranking is None:
        st.warning("No team ranking data available.")
    else:
        # Work on a copy so we do NOT mutate cached results
        all_teams_display_ranking = df_ranking.copy()

        # Ensure columns we plan to use exist and are strings
        if "Team name" in all_teams_display_ranking.columns:
            all_teams_display_ranking["Team name"] = all_teams_display_ranking["Team name"].astype(str)
        if "Player name" in all_teams_display_ranking.columns:
            all_teams_display_ranking["Player name"] = all_teams_display_ranking["Player name"].astype(str)

        # Build the "Team" column (prefer the nice HTML formatting if both source cols exist)
        if "Team name" in all_teams_display_ranking.columns and "Player name" in all_teams_display_ranking.columns:
            ...
            drop_cols = [c for c in ["Team name", "Player name"] if c in all_teams_display_ranking.columns]
            if drop_cols:
                all_teams_display_ranking = all_teams_display_ranking.drop(columns=drop_cols)
        else:
            # Fallback: if Team already exists, leave it; otherwise try to create from Team name if present
            if "Team" not in all_teams_display_ranking.columns and "Team name" in all_teams_display_ranking.columns:
                all_teams_display_ranking["Team"] = all_teams_display_ranking["Team name"].astype(str)

        # Now safely define cols
        cols = list(all_teams_display_ranking.columns)

        # Reorder so Rank first, Team second (if both exist)
        if "Rank" in cols and "Team" in cols:
            new_order = ["Rank", "Team"] + [c for c in cols if c not in ("Rank", "Team")]
            all_teams_display_ranking = all_teams_display_ranking[new_order]

        # Finally display as markdown (keeps bold + small gray player name HTML)
    ...
    st.text(f"\nBest Transfer Maker(s): \n{results["best_transfer_str"]} points earned in total through transfers\n\n")    
    st.text(f"\nWorst Transfer Maker(s): \n{results["worst_transfer_str"]} points earned in total through transfers\n\n")
    st.text(f"\nBiggest rank riser(s): \n{", ".join(results["rise_strs"])} \n\n")
    st.text(f"\nBiggest rank faller(s): \n{", ".join(results["fall_strs"])} \n\n")

    report_lines.append(f"\nBest Transfer Maker(s): \n{results["best_transfer_str"]} points earned in total through transfers\n\n")    
    report_lines.append(f"\nWorst Transfer Maker(s): \n{results["worst_transfer_str"]} points earned in total through transfers\n\n")
    report_lines.append(f"\nBiggest rank riser(s): {', '.join(results["rise_strs"])}\n\n")
    report_lines.append(f"\nBiggest rank faller(s): {', '.join(results["fall_strs"])}\n\n")

    st.text(f"\nTeams with the highest scoring captains:\n")
    st.table(results["top_3_highest_scoring_captains_df"].reset_index(drop=True))
    st.text(f"\nTeams with the lowest scoring captains:\n")
    st.table(results["bottom_3_lowest_scoring_captains_df"].reset_index(drop=True))

    report_lines.append("#### Top 3 Highest Scoring Captains")
    report_lines.append(results["top_3_highest_scoring_captains_df"].to_markdown(index=False))
    report_lines.append("#### Bottom 3 Lowest Scoring Captains")
    report_lines.append(results["bottom_3_lowest_scoring_captains_df"].to_markdown(index=False))

    final_report_md = "\n\n".join(report_lines)
    pdf_path, pdf_name = save_markdown_to_pdf(final_report_md, current_gw=results["current_gw"])
